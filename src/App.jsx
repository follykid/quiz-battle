...
const ROOM_TIMEOUT_MS = 60000;          // 原本 45s → 改 60s，給慢網路更多緩衝
const HEARTBEAT_MS = 15000;             // 原本 5s → 改 15s，大幅減少寫入量
const PRESENCE_TIMEOUT_MS = HEARTBEAT_MS * 6; // 原本 3倍 → 改 6倍（90s），避免誤踢
const USER_STATUS_HEARTBEAT_MS = 30000; // 原本 15s → 改 30s
const REVEAL_MS = 1200;
const AUTH_EMAIL_DOMAIN = 'sshes.tyc.edu.tw';
const TOTAL_TABLES = 14;
const MAX_MESSAGES = 50;               // 留言板最多監聽 50 則
...
// ─── 純工具函式移到元件外，避免每次 render 重建 ───────────────────────────

function calcWinRate(w = 0, l = 0) {
  const total = (w || 0) + (l || 0);
...
function questionKeyOf(text = '') {
  // 使用簡單 hash，避免 btoa 遇到中文字報錯
  let hash = 0;
...
// presence 判斷：拉長容忍時間到 6 倍心跳，避免網路延遲造成誤踢
function isAliveByUid(room, uid) {
  if (!uid) return false;
  if (uid === 'ai') return true;
  const ts = room?.presence?.[uid]?.ts || 0;
  return ts > 0 && Date.now() - ts <= PRESENCE_TIMEOUT_MS;
}
...
function getRoomDisplayStatus(room) {
  ...
  if (!room || room.gameOver || !room.p1Uid) return emptyStatus;
  ...
  if (aliveCount === 0) return emptyStatus;
...
function App() {
  ...
  // 遊戲畫面所需的資料全部從 roomData 派生，減少重複 state
  ...
  // ── 從 roomData 派生所有遊戲變數（單一資料來源）────────────────────────────
  const isAiMode       = roomData?.roomType === 'ai';
  const questions      = roomData?.roomQuestions || [];
  const currentIdx     = roomData?.currentIdx || 0;
  const selections     = roomData?.selections || null;
  const questionEndsAt = roomData?.questionEndsAt || 0;
  const gameOver       = !!roomData?.gameOver;
  const p1Score        = roomData?.scores?.p1 || 0;
  const p2Score        = roomData?.scores?.p2 || 0;
  const p1Name         = roomData?.p1 || '';
  const p2Name         = roomData?.p2 || '等待中...';
  const p1Id           = roomData?.p1Id || '';
  const p2Id           = roomData?.p2Id || '';
  const p2Joined       = !!roomData?.p2Uid;

  const p1Answered  = !!selections?.p1;
  const p2Answered  = !!selections?.p2;
  const bothAnswered = p1Answered && p2Answered;

  // timeLeft 仍需 state 驅動 UI 更新
  ...
  const dbSet = async (path, data) => {
    ...
    catch (err) { console.error('SET FAIL ->', path, err); throw err; }
  };

  const dbUpdate = async (path, data) => {
    try { await update(ref(db, path), data); }
    catch (err) { console.error('UPDATE FAIL ->', path, err); throw err; }
  };

  const dbRemove = async (path) => {
    try { await remove(ref(db, path)); }
    catch (err) { console.error('REMOVE FAIL ->', path, err); throw err; }
  };

  const dbPush = async (path, data) => {
    try { return await push(ref(db, path), data); }
    catch (err) { console.error('PUSH FAIL ->', path, err); throw err; }
  };

  const dbTx = async (path, updater) => {
    try { return await runTransaction(ref(db, path), updater); }
    catch (err) { console.error('TX FAIL ->', path, err); throw err; }
  };

  const dbRootUpdate = async (updates) => {
    try { await update(ref(db), updates); }
    catch (err) { console.error('ROOT UPDATE FAIL ->', updates, err); throw err; }
  };
  ...
  const leaveCurrentRoom = useCallback(async () => {
    ...
    try {
      ...
      if (room.p1Uid === user.uid) {
        // P1 離開 → 刪除整個房間
        await dbRemove(`rooms/${roomId}`).catch(console.error);
      } else if (room.p2Uid === user.uid) {
        // P2 離開 → 只清除自己的欄位
  ...
  const recordQuestionStat = async (questionObj, isCorrect) => {
    ...
    const correctAnswer = questionObj.options?.find((o) => o.isCorrect)?.text || '';
    ...
    await dbTx(`questionStats/${key}`, (stat) => {
      ...
      const prevWrongs   = current.wrongs   ?? 0;
      return {
        ...current,
        question: questionObj.question,
  ...
  useEffect(() => {
    if (!user?.uid) return;
    const offMe = onValue(ref(db, `users/${user.uid}`), (snap) => {
      const me = snap.val();
      if (!me) return;
      setUser((prev) => {
        if (!prev) return prev;
        return { ...prev, ...me, uid: prev.uid, studentId: me.studentId || prev.studentId };
      });
    }, console.error);
    return () => offMe();
  }, [user?.uid]);
  ...
  // ── 留言板（limitToLast 限制流量）────────────────────────────────────────

  useEffect(() => {
    if (!user?.uid) return;
    // 只監聽最新 MAX_MESSAGES 則，減少傳輸量
    const msgQuery = query(ref(db, 'messages'), limitToLast(MAX_MESSAGES));
    const offMessages = onValue(msgQuery, (snap) => {
      const val  = snap.val() || {};
    ...
    const offStats = onValue(ref(db, 'questionStats'), (snap) => {
      ...
      const list = Object.entries(val)
        .map(([id, v]) => {
          ...
          const wrongs   = v.wrongs   ?? 0;
          return {
            id, ...v, attempts, wrongs,
  ...
  useEffect(() => {
    ...
    const offConnected = onValue(connectedRef, async (snap) => {
      ...
      try {
        ...
        await disconnectOp.update({ online: false, lastSeen: serverTimestamp() });
        await dbUpdate(`users/${user.uid}`, { online: true, lastSeen: serverTimestamp() });
      } catch (err) {
    ...
    const timer = setInterval(() => {
      dbUpdate(`users/${user.uid}`, { online: true, lastSeen: serverTimestamp() }).catch(console.error);
    }, USER_STATUS_HEARTBEAT_MS);

    return () => {
      clearInterval(timer);
      if (disconnectOp) disconnectOp.cancel().catch(console.error);
      dbUpdate(`users/${user.uid}`, { online: false, lastSeen: serverTimestamp() }).catch(console.error);
      offConnected();
    };
  }, [user?.uid]);
  ...
  const handleLogin = async () => {
    ...
    if (!student) { alert('找不到此學號！'); return; }
    ...
    try {
      ...
      if (!snap.exists()) {
        ...
        finalUserData = { ...baseUserData, ...snap.val() };
  ...
  const startAiGame = async () => {
    if (Number(user?.hp) < 4) { alert('HP 不足 4 點！'); return; }
    ...
    await dbSet(`rooms/${tid}`, {
      roomType: 'ai',
      p1: user.name, p1Uid: user.uid, p1Id: user.studentId,
      p2: '🤖 練習用 AI', p2Uid: 'ai', p2Id: 'ai',
  ...
  const handleJoinTable = async (num) => {
    if (Number(user?.hp) < 2) { alert('HP 不足 2 點！'); return; }
    if (!allQuestions.length) return;
    ...
    try {
      result = await dbTx(`rooms/${tid}`, (room) => {
        ...
        const roomExpired = room.gameOver || !room.p1Uid || noLivePlayers
        ...
        if (!room.p2Uid || !p2Alive) {
          return {
            ...room,
            p2: user.name, p2Uid: user.uid, p2Id: user.studentId,
            lastActive: now,
            presence: { ...(room.presence || {}), [user.uid]: { online: true, ts: now } },
          };
        }
    ...
    if (!result?.committed) { alert('進房交易未成功寫入 Firebase'); return; }
    ...
    if (!finalRoom) { alert('Firebase 內找不到房間資料'); return; }

    const role = finalRoom.p1Uid === user.uid ? 'p1'
               : finalRoom.p2Uid === user.uid ? 'p2' : null;

    if (!role) { alert('此房間已滿或房間狀態尚未清除，請換桌或稍後再試'); return; }
  ...
  useEffect(() => {
    ...
    const offRoom = onValue(roomRef, (snap) => {
      ...
      if (!data) {
        if (view === 'game') { stopAllAudio(); resetGameState(); setView('lobby'); }
        return;
      }

      const roleFromDb = data.p1Uid === user.uid ? 'p1'
                       : data.p2Uid === user.uid ? 'p2' : null;

      if (!roleFromDb) { stopAllAudio(); resetGameState(); setView('lobby'); return; }
  ...
  useEffect(() => {
    ...
    // 心跳間隔已從 5s 拉長到 15s，大幅減少 Firebase 寫入量
    const timer = setInterval(() => {
      dbSet(presencePath, { online: true, ts: Date.now() }).catch(console.error);
  ...
  const advanceToNextQuestion = useCallback(async (expectedIdx) => {
    if (!roomId) return;
    try {
  ...
  useEffect(() => {
    if (!roomId || myRole === 'viewer' || !roomData || gameOver) return;
    if (!bothAnswered) return;

    const key = `${roomId}-${currentIdx}`;
    ...
    const timerId = setTimeout(async () => {
      advanceTimerRef.current = null;
      await advanceToNextQuestion(currentIdx);
    }, REVEAL_MS);
    ...
    return () => {
      ...
      if ((roomDataRef.current?.currentIdx ?? 0) === currentIdx) {
        isSwitching.current    = false;
        advanceLockRef.current = '';
      }
    };
  }, [roomId, myRole, currentIdx, gameOver, bothAnswered, advanceToNextQuestion, roomData]);
  ...
  useEffect(() => {
    ...
    // 修正：對手必須確認「確實已死亡」才強制推進，避免因心跳延遲誤踢
  ...
  }, [roomId, myRole, roomData, gameOver, questionEndsAt, bothAnswered,
      p1Answered, p2Answered, user?.uid]);

  // ── AI 自動答題 ───────────────────────────────────────────────────────────

  useEffect(() => {
    if (!isAiMode || !roomId || gameOver) return;
    ...
    const timer = setTimeout(async () => {
      ...
      await dbTx(`rooms/${roomId}`, (room) => {
        ...
        return {
          ...
          selections: {
            ...(room.selections || {}),
            p2: { text: aiOpt?.text || '', isCorrect: !!aiOpt?.isCorrect },
          },
          scores: {
  ...
  }, [isAiMode, roomId, gameOver, currentIdx, p1Answered, p2Answered]);
  ...
  const onSelect = async (opt) => {
    ...
    if (opt.isCorrect) { correctSfx.current.currentTime = 0; correctSfx.current.play().catch(console.error); }
    else               { wrongSfx.current.currentTime = 0;   wrongSfx.current.play().catch(console.error); }

    // 只對自己的選擇子節點寫入，不對整個房間做 transaction，減少競爭
    const result = await dbTx(`rooms/${roomId}`, (room) => {
      ...
      return {
        ...
        selections: {
          ...(room.selections || {}),
          [role]: { text: opt.text, isCorrect: !!opt.isCorrect },
        },
        scores: {
          ...(room.scores || { p1: 0, p2: 0 }),
  ...
  const finishGameAndGoLobby = async () => {
    ...
    const rewardPoints = isAiMode
      ? (isWin ? Math.floor(myScore * 0.5) : Math.floor(myScore * 0.3))
      : (isWin ? myScore : Math.floor(myScore * 0.8));
      ...
      <div style={{ display: 'flex', gap: '5px' }}>
        <input
          ...
          style={{
            ...
            background: '#333', border: 'none', color: 'white', minWidth: 0,
          }}
          placeholder="輸入聊天..."
        />
    ...
    <div className="safe-container">
      <style>{`
        ...
        .box {
          ...
          border: 1px solid #333; margin-bottom: 10px; box-sizing: border-box;
        }
        .btn {
        ...
        .avatar-lg {
          ...
          border: 3px solid #ffeb3b; background: #333; flex-shrink: 0;
        }
        .rank-table {
          width: 100%; border-collapse: collapse;
          font-size: 0.85rem; table-layout: auto;
        }
        .rank-table th {
          text-align: left; color: #888; border-bottom: 1px solid #444;
          padding: 5px 8px; white-space: nowrap;
        }
        .rank-table td {
          padding: 8px 8px; border-bottom: 1px solid #222;
          vertical-align: middle; white-space: nowrap;
        }
        .lobby-layout {
          display: grid; grid-template-columns: 380px 1fr;
          gap: 20px; max-width: 1260px; margin: 0 auto;
        ...
        .msg-box {
          height: 300px; overflow-y: auto; background: #111;
          padding: 15px; border-radius: 10px; margin-bottom: 10px;
          border: 1px solid #333; display: flex; flex-direction: column;
          box-sizing: border-box;
        }
        .option-btn {
          padding: 18px; font-size: 1.1rem; border-radius: 12px;
          border: none; color: white; background: #333;
          margin-bottom: 10px; width: 100%; text-align: left;
          cursor: pointer; box-sizing: border-box; word-break: break-word;
        }
        input, button, textarea, select { font-size: 16px; }

        @media (max-width: 980px) {
          .lobby-layout { grid-template-columns: 1fr; gap: 12px; padding: 8px; }
        }
        ...
        @media (max-width: 480px) {
          header { padding: 10px; }
          .box { padding: 12px; }
          .msg-box { height: 220px; padding: 10px; }
      ...
      {/* ── 登入畫面 ── */}
      {view === 'login' && (
        <div style={{ padding: '40px 12px', textAlign: 'center', width: '100%', boxSizing: 'border-box' }}>
          <h1>⚔️ 知識對戰系統</h1>
          <div className="box" style={{ maxWidth: '360px', width: '100%', margin: '0 auto', boxSizing: 'border-box' }}>
            <div style={{
              background: '#111', border: '1px solid #333', borderRadius: '10px',
              padding: '12px 14px', marginBottom: '18px', lineHeight: 1.8,
            ...
            <input
              placeholder="學號" value={loginId} onChange={(e) => setLoginId(e.target.value)}
              style={{ width: '100%', padding: '12px', marginBottom: '15px', background: '#111', color: 'white', border: '1px solid #444', borderRadius: '8px', boxSizing: 'border-box' }}
            />
            <input
              type="password" placeholder="密碼" value={loginPwd} onChange={(e) => setLoginPwd(e.target.value)}
              style={{ width: '100%', padding: '12px', marginBottom: '25px', background: '#111', color: 'white', border: '1px solid #444', borderRadius: '8px', boxSizing: 'border-box' }}
            />
            <button className="btn" onClick={handleLogin} style={{ width: '100%', background: '#4caf50', color: 'white' }}>
              登入
            </button>
          </div>
        </div>
        ...
        <>
          <header>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
              ...
              {/* HP 最低顯示 0 */}
              <span style={{ color: '#ff5252', marginLeft: '5px' }}>❤️ {Math.max(0, user.hp)}</span>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {!user?.isTeacher && (
                <button
                  onClick={async () => {
                    if (user.totalScore < 15) { alert('積分不足'); return; }
                    await dbUpdate(`users/${user.uid}`, { totalScore: increment(-15), hp: increment(1) }).catch(console.error);
                    alert('兌換成功');
                  }}
                  className="btn"
                  style={{ background: '#4caf50', color: 'white', padding: '5px 10px', fontSize: '0.8rem' }}
                >
                  +1HP(15分)
                </button>
              )}
              <button
                onClick={async () => {
                  if (user?.uid) {
                    await dbUpdate(`users/${user.uid}`, { online: false, lastSeen: serverTimestamp() }).catch(console.error);
                  }
                ...
                className="btn"
                style={{ background: '#555', color: 'white', padding: '5px 10px', fontSize: '0.8rem' }}
              >
                登出
              </button>
            </div>
          ...
          <main style={{ width: '100%', boxSizing: 'border-box', paddingTop: '12px' }}>
            ...
            {view === 'lobby' && user?.isTeacher && (
              <div style={{ maxWidth: '1200px', width: '100%', margin: '0 auto', padding: '8px', boxSizing: 'border-box' }}>
                <div className="box">
                  <h3 style={{ color: '#ffeb3b', textAlign: 'center', marginTop: 0 }}>📊 學生容易錯的題目</h3>
                  {questionStatsList.length === 0 ? (
                    <div style={{ textAlign: 'center', color: '#aaa', padding: '20px 0' }}>目前尚無統計資料</div>
                  ) : (
                    <div style={{ overflowX: 'auto' }}>
                      <table className="rank-table" style={{ fontSize: '0.95rem', minWidth: '820px' }}>
                        <thead>
                          <tr>
                            <th>#</th><th>題目</th><th>正解</th>
                            <th>作答次數</th><th>答錯次數</th><th>錯誤率</th>
                          </tr>
                        </thead>
                        <tbody>
                          {questionStatsList.map((q, i) => (
                            <tr key={q.id}>
                              <td>{i + 1}</td>
                              <td style={{ maxWidth: '520px', whiteSpace: 'normal', lineHeight: 1.6 }}>{q.question}</td>
              ...
              <div className="lobby-layout">
                <div className="box">
                  ...
                  <div style={{ overflowX: 'auto', width: '100%' }}>
                    <table className="rank-table" style={{ minWidth: '620px' }}>
                      <thead>
                        <tr>
                          <th>#</th><th>狀態</th><th>頭像</th><th>姓名</th>
                          <th>積分</th><th>勝</th><th>敗</th><th>勝率</th>
                        </tr>
                      </thead>
                      <tbody>
                        {leaderboard.map((u, i) => (
                          <tr key={u.uid}>
                            <td>{i + 1}</td>
                            <td>
                              <span title={u.online ? '上線中' : '離線'} style={{
                            ...
                            <td>
                              <img src={avatarSrc(u.studentId)} className="avatar" alt=""
                                onError={(e) => { e.target.src = 'https://via.placeholder.com/40'; }} />
                            </td>
                            <td>{u.name}</td>
                ...
                <div>
                  <div className="box" style={{ textAlign: 'center', border: '1px solid #ff5252' }}>
                    <button className="btn" onClick={startAiGame}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(80px, 1fr))', gap: '8px' }}>
                      {Array.from({ length: TOTAL_TABLES }).map((_, i) => {
                        ...
                        const status = getRoomDisplayStatus(roomsData[tid]) || {
                          count: 0, label: '空房', people: '0/2人',
                          bg: '#2c2c2c', border: '#444', shadow: 'transparent',
                        };
                        return (
                          <button key={i} className="btn" onClick={() => handleJoinTable(i + 1)}
                            style={{
            ...
            {/* ── 遊戲畫面 ── */}
            {view === 'game' && (
              <div style={{ maxWidth: '800px', width: '100%', margin: '0 auto', padding: '8px', boxSizing: 'border-box' }}>
                <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                  <div style={{ fontSize: '3rem', fontWeight: 'bold' }}>{timeLeft}s</div>
                  <div style={{ marginBottom: '12px' }}>
                    <button className="btn"
                      onClick={async () => { await leaveCurrentRoom(); stopAllAudio(); resetGameState(); setView('lobby'); }}
                      style={{ background: '#666', color: 'white', padding: '10px 18px', fontSize: '0.95rem' }}>
                      離開房間
                    </button>
                  </div>
                  <div style={{
                    display: 'flex', justifyContent: 'space-around', alignItems: 'center',
                    gap: '12px', flexWrap: 'wrap', background: '#1e1e1e',
                    padding: '15px', borderRadius: '15px', border: '1px solid #444',
                  }}>
                    <div style={{ minWidth: 100 }}>
                      <img src={avatarSrc(p1Id, 80)} className="avatar-lg" alt=""
                    ...
                    <div style={{ minWidth: 100 }}>
                      <img src={avatarSrc(p2Id, 80)} className="avatar-lg" alt=""
                        onError={(e) => { e.target.src = 'https://via.placeholder.com/80'; }} />
                        ...
                        <button key={i} onClick={() => onSelect(opt)}
                          ...
                          style={{
                            background: selections?.[myRole]
                              ? opt.isCorrect
                                ? '#2e7d32'
                                : selections[myRole].text === opt.text ? '#c62828' : '#333'
                              : '#333',
                          }}>
                          {opt.text}
                        </button>
          ...
          {gameOver && (
            <div style={{
              ...
              zIndex: 2000, padding: '20px', boxSizing: 'border-box', textAlign: 'center',
            }}>
              <h1 style={{
                fontSize: 'clamp(2.2rem, 10vw, 4rem)',
                color: (myRole === 'p1' ? p1Score : p2Score) > (myRole === 'p1' ? p2Score : p1Score)
                  ? '#ffeb3b' : '#ff5252',
                marginBottom: '20px',
              }}>
                {(myRole === 'p1' ? p1Score : p2Score) > (myRole === 'p1' ? p2Score : p1Score)
                  ? 'VICTORY! 🎉' : 'DEFEAT... 💀'}
              </h1>
              <button className="btn" onClick={finishGameAndGoLobby}
                style={{ background: '#4caf50', color: 'white', padding: '15px 32px', fontSize: '1.1rem', maxWidth: '100%' }}>
                返回大廳
              </button>
            </div>
          )}
